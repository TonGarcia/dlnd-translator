...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    """
    # Returned Variables initialized
    source_id_text = []
    target_id_text = []
   
    # Texts converted into setences (using \n as separator)
    source_text_sentences = source_text.split('\n')
    target_text_sentences = target_text.split('\n')
    
    # Iterate sentence per sentence splitting words per sentence id FOR INPUTS
...
def model_inputs():
    ...
    """
    inputs = tf.placeholder(tf.int32, [None, None], name='input') # Rank 2 Tensor
    targets = tf.placeholder(tf.int32, [None, None], name='targets') # Rank 2 Tensor
    learning_rate = tf.placeholder(tf.float32, None, name='learning_rate') # Rank 0 Tensor (scalar)
    keep_prob = tf.placeholder(tf.float32, None, name='keep_prob') # 1 Dimension Float Tensor (scalar)
    target_seq_len = tf.placeholder(tf.int32, None, name='target_sequence_length') # Rank 1 Tensor
    source_seq_len = tf.placeholder(tf.int32, None, name='source_sequence_length') # Rank 1 Tensor
    max_target_seq_leng = tf.reduce_max(target_seq_len)
    return inputs, targets, learning_rate, keep_prob, target_seq_len, max_target_seq_leng, source_seq_len
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob, 
    ...
    def make_cell(rnn_size):
        ...
        # TODO: read it updated doc, it is saying that input_size is deprecated
    ...
    rnn_enc_output, rnn_enc_state = tf.nn.dynamic_rnn(enc_cell, enc_embed_input, 
...
# RNN Size
rnn_size = 256 # RNN Size is also small. You should use a larger RNN size in order to give the network enough power to learn the intricacies of the language. Please try RNN size in the range 128 - 256.
# Number of Layers
num_layers = 2
# Embedding Size
